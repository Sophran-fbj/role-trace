...
import { EVIDENCE_SCHEMA_VERSION, evidenceExtractionSchema } from "@/lib/ai/schemas";
...
export async function extractEvidence(documents: SourceDocument[]) {
  ...
  const proposal = await requestStructured({ name: "applylens_evidence", schema: evidenceExtractionSchema, instructions, input: { sourceBlocks: sourceBlocks.map(({ id, documentId, text }) => ({ id, documentId, text })) } });
  const seen = new Set<string>();
  const evidence: EvidenceItem[] = [];
  let discarded = 0;
  for (const item of proposal.evidence) {
    const fingerprint = `${item.sourceBlockId}|${item.exactQuote}`.toLowerCase();
    if (seen.has(fingerprint) || !validEvidence(item, sourceBlocks)) { discarded += 1; continue; }
    seen.add(fingerprint);
    evidence.push({ ...item, id: crypto.randomUUID(), reviewState: "pending" });
  }
  return { sourceBlocks, evidence, discarded, metadata: { promptVersion: EVIDENCE_SCHEMA_VERSION, schemaVersion: EVIDENCE_SCHEMA_VERSION, model: process.env.OPENAI_MODEL ?? "unconfigured" } };
}
